...
def move():
    ...
    for x in range(n):
        for y in range(n):
            if grid[x][y] != 0:
                direction = directions[grid[x][y] - 1]
                found = False
                
                # 상어의 위치인 경우
                for idx in priorities[grid[x][y] - 1][direction - 1]:
                    nx = x + dx[idx - 1]
                    ny = y + dy[idx - 1]
                    ...
                    if smell[nx][ny][1] == 0:  # 냄새가 나지 않는 곳이라면
                        directions[grid[x][y] - 1] = idx
                        
                        # 상어 이동시키기
                        if new_data[nx][ny] == 0:
                            new_data[nx][ny] = grid[x][y]
                        else:
                            new_data[nx][ny] = min(grid[x][y], new_data[nx][ny])
                ...
                for idx in priorities[grid[x][y] - 1][direction - 1]:
                    ...
                    if smell[nx][ny][0] == grid[x][y]:  # 자신의 냄새가 있는 곳이라면
                        # 해당 상어 방향 변경
                        directions[grid[x][y] - 1] = idx
                        # 상어 이동시키기
                        new_data[nx][ny] = grid[x][y]
